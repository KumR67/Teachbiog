...
function parseQuery(raw) {
    ...
    if(exactMatch) return { exact: exactMatch[1], terms: [] };
...
function highlightField(html, exact, terms){
    let result=html;
    if(exact){ const re=new RegExp(exact,'gi'); result=result.replace(re,m=>`<span class="match">${m}</span>`); }
    if(terms && terms.length) terms.forEach(t=>{ if(t.regex) result=result.replace(t.regex,m=>`<span class="match">${m}</span>`); });
    return result;
}
